...
def aggregate_attribution(rows: Iterable[dict]) -> dict[str, AttributionMetric]:
    ...
    return {k: AttributionMetric(int(v["clicks"]), int(v["conversions"]), round(v["commission"], 2),
                                 round(v["clicks"] / v["impressions"], 6) if v["impressions"] else 0.0,
                                 round(v["conversions"] / v["clicks"], 6) if v["clicks"] else 0.0,
                                 round(v["commission"] / v["clicks"], 6) if v["clicks"] else 0.0)
...
def portfolio_summary(metrics: dict[str, AttributionMetric]) -> dict[str, float]:
    ...
    return {"clicks": clicks, "conversions": conversions, "commission": round(commission, 2),
            "cvr": round(conversions / clicks, 6) if clicks else 0.0,
            "epc": round(commission / clicks, 6) if clicks else 0.0}
